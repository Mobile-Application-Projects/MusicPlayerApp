...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, AllEbookFragment.createDataParse, FavEbookFragment.createDataParsed, CurrentEbookFragment.createDataParsed {
    ...
    private void init() {
        ...
        FloatingActionButton refreshEbooks = findViewById(R.id.btn_refresh);
        seekbarController = findViewById(R.id.seekbar_controller);
        viewPager = findViewById(R.id.ebooks_viewpager);
        ...
        refreshEbooks.setOnClickListener(this);
    ...
    private void about() {
        ...
        builder.setTitle(getString(R.string.about))
                .setMessage("This Application was created by Rimya Fareed, M.Saim Hassan & Marium Ahmed. ")
                ...
                if (checkFlag)
                    if (mediaPlayer != null) {
                        if (favFlag) {
                            ...
                            EbooksList favList = new EbooksList(ebooksList.get(currentPosition).getTitle(),
                                    ebooksList.get(currentPosition).getSubTitle(), ebooksList.get(currentPosition).getPath());
                            FavoritesOperations favoritesOperations = new FavoritesOperations(this);
                            favoritesOperations.addEbookFav(favList);
                            setPagerLayout();
                            favFlag = false;
                        } else {
        ...
        switch (v.getId()) {
            case R.id.img_btn_play:
                if (checkFlag) {
                    ...
                    Toast.makeText(this, "Select the Ebook ..", Toast.LENGTH_SHORT).show();
                }
                break;
                ...
                if (checkFlag) {
                    if (mediaPlayer.getCurrentPosition() > 10) {
                        if (currentPosition - 1 > -1) {
                            attachEbook(ebooksList.get(currentPosition - 1).getTitle(), ebooksList.get(currentPosition - 1).getPath());
                            currentPosition = currentPosition - 1;
                        } else {
                            attachEbook(ebooksList.get(currentPosition).getTitle(), ebooksList.get(currentPosition).getPath());
                        }
                    } else {
                        attachEbook(ebooksList.get(currentPosition).getTitle(), ebooksList.get(currentPosition).getPath());
                    }
                } else {
                    Toast.makeText(this, "Select an Ebook . .", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.img_btn_next:
                if (checkFlag) {
                    if (currentPosition + 1 < ebooksList.size()) {
                        attachEbook(ebooksList.get(currentPosition + 1).getTitle(), ebooksList.get(currentPosition + 1).getPath());
                        currentPosition += 1;
                    } else {
    ...
    /**
     * Function to attach the Ebook to the Ebook player
     *
     * @param name
     * @param path
     */

    private void attachEbook(String name, String path) {
        imgBtnPlayPause.setImageResource(R.drawable.play_icon);
        setTitle(name);
    ...
    /**
     * Function to set the controls according to the Ebook
     */
    ...
    /**
     * Function to play the Ebook using a thread
     */
    ...
    public void onDataPass(String name, String path) {
        ...
        attachEbook(name, path);
    }

    @Override
    public void getLength(int length) {
        this.allEbooksLength = length;
    }

    @Override
    public void fullEbooksList(ArrayList<EbooksList> ebooksList, int position)
    {
        this.ebooksList = ebooksList;
        this.currentPosition = position;
        this.playlistFlag = ebooksList.size() == allEbooksLength;
